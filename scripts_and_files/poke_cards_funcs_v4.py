...
def new_pokemon_df(name_of_csv):
    ...
    with the following columns: 'Card_Name', 'Set_Name', 'url', 'ungraded_price', 'PSA10_price', 'grade_yn'
    ...
    """
    poke_df = pd.DataFrame(columns = ['Card_Name', 'Set_Name', 'url', 'ungraded_price', 'PSA10_price', 'grade_yn'])
...
def add_card(df_poke_cards, url, poke_csv_name):
    ...
    if ((df_poke_cards['Card_Name'] == check_card_name) & (df_poke_cards['Set_Name'] == set_name_only)).any():
        ...
        choice_input = input("Would you like to update price instead? - ")
        if choice_input == 'y' or choice_input == 'yes' or choice_input == 'Yes':
            df_poke_cards = update_price(df_poke_cards, check_card_name, set_name_only, poke_csv_name, call_flag = 1)
        else:
        ...
        new_row = {'Card_Name': check_card_name, 'Set_Name': set_name_only, 'url': url, \
                   'ungraded_price':ungraded_price, 'PSA10_price':psa10_price, 'grade_yn':grade_choice}
