...
This recipe guides you through the Pokedex binder script. I use a csv file with the Pokémon national dex numbers to help decide where it belongs in your binder (found in `scripts_and_files`). A csv file is outputted for your record keeping.

## Version History

- `page_sorting_v2.py` (v2) - November 2025 by W. B. Salazar
...
For this procedure, we solely need to run Python scripts and functions. I primarily used Jupyter Notebooks from Anaconda to run this whole process. I currently don't have an easy website to run this from, but the example notebooks are simple enough.
...
$$page_number = poke_national_dex_num // 9$$

and 

$$page_index = poke_national_dex_num % 9$$ 
